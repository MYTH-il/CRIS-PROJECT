...
Generates ~60,000 FIR-style India-context crime report records.
...
IPC_SECTIONS = {
    "Theft": [
        {"section": "378", "title": "Theft"},
        {"section": "379", "title": "Punishment for Theft"},
        {"section": "380", "title": "Theft in Dwelling House"},
        {"section": "381", "title": "Theft by Clerk/Servant"},
        {"section": "411", "title": "Dishonestly Receiving Stolen Property"},
    ],
    "Assault": [
        {"section": "323", "title": "Voluntarily Causing Hurt"},
        {"section": "324", "title": "Hurt by Dangerous Weapons"},
        {"section": "325", "title": "Grievous Hurt"},
        {"section": "326", "title": "Grievous Hurt by Dangerous Weapons"},
        {"section": "352", "title": "Assault or Criminal Force"},
    ],
    "Fraud": [
        {"section": "415", "title": "Cheating"},
        ...
        {"section": "471", "title": "Using Forged Document as Genuine"},
    ],
    "Murder": [
        {"section": "302", "title": "Punishment for Murder"},
        {"section": "304", "title": "Culpable Homicide Not Amounting to Murder"},
        {"section": "307", "title": "Attempt to Murder"},
    ],
    "Kidnapping": [
        {"section": "359", "title": "Kidnapping"},
        {"section": "361", "title": "Kidnapping from Lawful Guardianship"},
        {"section": "362", "title": "Abduction"},
        {"section": "364A", "title": "Kidnapping for Ransom"},
    ],
    "Robbery": [
        {"section": "392", "title": "Punishment for Robbery"},
        {"section": "394", "title": "Hurt During Robbery"},
        {"section": "397", "title": "Robbery with Attempt to Cause Death or Grievous Hurt"},
    ],
    "Drug Offence": [
        {"section": "272", "title": "Adulteration of Food or Drink Intended for Sale"},
        {"section": "273", "title": "Sale of Noxious Food or Drink"},
    ],
    "Cybercrime": [
        {"section": "420", "title": "Cheating and Dishonestly Inducing Delivery of Property"},
        {"section": "468", "title": "Forgery for Purpose of Cheating"},
    ...
    "Sexual Offence": [
        {"section": "354", "title": "Assault to Outrage Modesty"},
        {"section": "354A", "title": "Sexual Harassment"},
        {"section": "376", "title": "Punishment for Rape"},
    ],
    "Property Damage": [
        {"section": "427", "title": "Mischief Causing Damage"},
        {"section": "436", "title": "Mischief by Fire/Explosive Substance"},
        {"section": "447", "title": "Criminal Trespass"},
    ],
}

SPECIAL_ACTS = {
    "Cybercrime": ["IT Act 66C", "IT Act 66D"],
    "Drug Offence": ["NDPS Act 20", "NDPS Act 21"],
}
...
INDIAN_NAMES = [
    "Aarav Sharma", "Ananya Iyer", "Rahul Verma", "Sneha Patil", "Vikram Singh",
    "Neha Gupta", "Suresh Reddy", "Priya Nair", "Rohit Kulkarni", "Meera Joshi",
    "Irfan Khan", "Farhan Ali", "Kavya Rao", "Arjun Mehta", "Nisha Kapoor",
]
FIR_TEMPLATES = [
    ("The complainant, {complainant_name}, states that on {date} at {time}, "
     "at {location}, {summary} {weapon_line} {ipc_line}"),
    ("On {date} at about {time}, within the jurisdiction of {station} PS, "
     "the complainant reported that {summary} {ipc_line}"),
    ("It is reported by {complainant_name} that at {time} on {date}, "
     "near {location}, {summary} {weapon_line} {ipc_line}"),
]
...
def pick_ipc_sections(crime_type):
    ipc_list = IPC_SECTIONS.get(crime_type, [])
    if not ipc_list:
        return [], []
    count = 1 if len(ipc_list) == 1 else random.choice([1, 2])
    picks = random.sample(ipc_list, k=count)
    sections = [p["section"] for p in picks]
    titles = [p["title"] for p in picks]
    return sections, titles

def generate_incident_description(crime_type, subtype, area_type, district, weapon, report_date, report_time, station_code):
    base = random.choice(CRIME_TYPES[crime_type]["descriptions"])
    weapon_line = ""
    if weapon != "None":
        weapon_line = f"A {weapon.lower()} was reportedly used during the commission of the offence."
    if area_type == "Rural":
        base += " The incident occurred in a rural locality with limited street lighting."
    elif area_type == "Industrial":
        base += " The location falls within the industrial zone with minimal civilian presence during the incident time."

    ipc_sections, ipc_titles = pick_ipc_sections(crime_type)
    ipc_line = ""
    if ipc_sections:
        ipc_joined = "/".join(ipc_sections)
        ipc_line = f"Registered under IPC Sections {ipc_joined}."

    template = random.choice(FIR_TEMPLATES)
    complainant_name = random.choice(INDIAN_NAMES)
    location = f"{random.choice(LANDMARKS)}, {district}"
...
for i in range(NUM_RECORDS):
    ...
    description, ipc_sections, ipc_titles = generate_incident_description(
        crime_type, subtype, area_type, district, weapon, report_date, report_time, station_code
    )
    ...
    ipc_section_text = "/".join(ipc_sections) if ipc_sections else ""
    ipc_title_text = ", ".join(ipc_titles) if ipc_titles else ""
    special_act = ""
    if crime_type in SPECIAL_ACTS:
        special_act = random.choice(SPECIAL_ACTS[crime_type])
    ...
    records.append({
        ...
        "ipc_sections": ipc_section_text,
        "ipc_section_titles": ipc_title_text,
